...
In vSphere, you can add a total of 4 network adapters on the appliance. Additional adapters will be marked VCHA.

Make sure that the hostname specified to access vCenter, resolves within the subnet as the VCHA ip address.
...
Add the dns entries for OpenShift, .5 for API, and the vSphere VCHA ip from previous step:
...
10.0.0.44 vcsa.pietersmalan.com
----
