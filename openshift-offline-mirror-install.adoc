...
Start-Service sshd
New-NetFirewallRule -Name "SSH" -DisplayName "SSH" -Description "Allow SSH" -Profile Any -Direction Inbound -Ac...
New-NetFirewallRule -Name "OCPLOGS" -DisplayName "OCPLOGS" -Description "Allow OCP Log Collection" -Profile Any...
ssh-keygen
$authorizedkry = Get-Content -Path $env:USERPROFILE\.ssh\id_rsa.pub
$remotePowerShell = "powershell Add-Content -Force -Path $env:ProgramData\ssh\administrators_authorized_keys -V...
cd .\.ssh\
