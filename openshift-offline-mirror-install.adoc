...
To copy images to mirror, use skopeo, version 1.10, or later:

[code]
----
skopeo --override-os windows copy docker://mcr.microsoft.com/windows/servercore:ltsc2022 dir:servercore
skopeo --override-os windows copy dir:servercore docker://bastion.pietersmalan.com:8443/windows/servercore:ltsc2022

skopeo --override-os windows copy docker://quay.io/revoman/webserver:latest dir:webserver
skopeo --override-os windows copy dir:webserver docker://bastion.pietersmalan.com:8443/revoman/webserver:latest
----

Alternative, if you don't have access to latest version, you can use a container:

[code]
----
sudo podman run --privileged --rm -v $PWD:/var/lib/containers/storage:z quay.io/skopeo/stable copy  --override-os windows docker://mcr.microsoft.com/windows/servercore:ltsc2022 dir:/var/lib/containers/storage/servercore
sudo podman run --privileged --rm -t -v $PWD:/var/lib/containers/storage:z quay.io/skopeo/stable copy --override-os windows --dest-creds quay:quayquay --dest-tls-verify=false dir:/var/lib/containers/storage/servercore docker://bastion.pietersmalan.com:8443/windows/servercore:ltsc2022
----

Sample deployment, using https://github.com/therevoman/windows-containers/tree/main/simple-webserver - therevoman 
...
[note]
nerdctl.exe is a utility to examine images on Windows node. See https://github.com/containerd/nerdctl. Nampespace is required - for example - c:\temp\nerdctl.exe images --namespace k8s.io
