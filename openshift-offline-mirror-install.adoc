...
Modified contents of imageset-config.yaml, produced by the command above with required images for wmco.
...
[start=4]
. Take not of the location of the  *ImageContentSourcePolicy* :
[code]
----
...
Edit install-config.yaml and add the *imageContentSourcePolicy* as captured during the mirroring process, by concatinating all the mirrors (everything under *repositoryDigestMirrors:*), and make sure to change the NetworkType to OVNKubernetes from OpenShiftSDN:
