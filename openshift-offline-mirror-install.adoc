...
In this example - 10.0.0.2 (ens224) represents the host within private lan, and 192.68.89.102 (accessible through ens192) represents the vSphere Server. 

Execute the following on the host within the private lan as root:
[code]
----
firewall-cmd --permanent --zone=public --add-interface=ens192
firewall-cmd --permanent --zone=internal --add-interface=ens224
firewall-cmd --zone=internal --add-service=https --add-service=dns --add-service=dhcp --ad-service=ssh--permanent
firewall-cmd --permanent --new-policy VCSAPolicy
firewall-cmd --permanent --policy=VCSAPolicy --add-ingress-zone=internal
firewall-cmd --permanent --policy=VCSAPolicy --add-egress-zone=ANY
firewall-cmd --permanent --policy=VCSAPolicy --add-rich-rule='rule family="ipv4" destination address="10.0.0.2" forward-port port="443" protocol="tcp" to-port="443" to-addr="192.168.89.102"'
firewall-cmd --reload
